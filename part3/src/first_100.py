...
class thr(threading.Thread):
    ...
    def run(self):
        ...
        global src
        for i in range(0,40):
            status, output = commands.getstatusoutput("python Part3.py " + sys.argv[1] + " modularityclass.csv"  + " " + str(devices[src[i]]) + " " + str(self.k))
        ...
        if len(data_2):
            ...
            file2.flush()
        file3.write(str(self.k) + "," + str(np.mean(data_3)) +","+ str(np.std(data_3)) +'\n')
        file3.flush()
        print self.k
